...
This is a project  mainly for detecting keypoints of  face picture with deep learning.  

First, the method `CascadeClassifier` in `opencv `module  would be used to detect  faces in a picture;
...
```python
x = self.dense3(x) # shoul NOT be x = F.relu(self.dense3(x))
```

That is the bit that caused my results to be off. Facial keypoints/landmarks is a regression problem and not a classification problem. **So therefore no activation is needed before the last output**. What is misleading that the paper speaks about 'linear activation' - but that's quite a misleading name, it would be better to call it identity activation or 'do nothing' activation or just no activation.So here the code should be `x = self.dense3(x)`.



THE BELOW is the official project description, I keep it for better understanding.
